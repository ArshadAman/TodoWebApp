...
from datetime import datetime #For date and time of the task
...
class User(db.Model, UserMixin):
    ...
    #Added Name Feild in the database
...
def signup():
    if request.method == "POST":
        ...
        name = request.form.get("name") #Get the name of the user
        if User.query.filter_by(email=email).first():
            ...
            new_user = User(
                ...
                name = name,#Registered the user
            )
            db.session.add(new_user)
            db.session.commit()
            login_user(new_user) #Login the new user after registration
            return redirect(url_for("home")) #Redirect user to home after login
